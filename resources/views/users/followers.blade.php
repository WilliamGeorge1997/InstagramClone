            ...
            <tbody>
                @foreach ($followersData as $followerData)
                    <tr>
                        ...
                        <td><a href="{{ route('users.show', $followerData->id) }}"
                                class="text-decoration-none text-dark">{{ $followerData->fullname }}<br><span
                                    class="text-muted">{{ $followerData->username }}</span></a></td>
                        <td>
                            @if (auth()->check() && auth()->user()->id !== $followerData->id)
                                @if (auth()->user()->isFollowing($followerData))
                                    {{-- Unfollow button --}}
                                    <form action="{{ route('users.unfollow', $followerData->id) }}" method="post"
                                        class="d-inline">
                                        @csrf
                                        @method('delete')
                                ...
                                @else
                                    ...
                                    <form action="{{ route('users.follow', $followerData->id) }}" method="post"
                                        class="d-inline">
                                        @csrf
