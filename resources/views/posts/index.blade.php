    ...
    @else
        @foreach ($posts as $post)
            ...
            @endif
                ...
                <a href="{{ route('posts.show', ['post' => $post->id]) }}">
                                ...
                                @foreach ($post->media as $key => $medium)
                                    <div class="carousel-item {{ $key == 0 ? 'active' : '' }}" style="height:500px;">
                                        ...
                                        @if (in_array($extension, ['jpg', 'jpeg', 'png', 'gif']))
                                            <img loading="lazy" src="{{ Storage::url($medium->media) }}" class="d-block post-image w-100"
                                                style="height:500px;">

                                            alt="...">
                                        @elseif (in_array($extension, ['mp4', 'mov', 'avi', 'wmv']))
                                            <video class="d-block post-video w-100" style="height:500px;">
                                                autoplay muted
                                                loop>
                    ...
                    @else
                        <div>
                            ...
                            @endphp
                            @if (in_array($extension, ['jpg', 'jpeg', 'png', 'gif']))
                                <img loading="lazy" src="{{ Storage::url($post->media->first()->media) }}"
                                    class="d-block post-image w-100" style="object-fit: cover; " alt="...">
                            @elseif (in_array($extension, ['mp4', 'mov', 'avi', 'wmv']))
                                <video class="d-block post-video w-100" style="object-fit: cover; " autoplay muted loop>
                    ...
                    <div class="d-flex  mt-2 justify-content-between">
                            ...
                            @auth
                                @if (auth()->user()->hasLiked($post))
                                    ...
                                    <form action="{{ route('posts.unlike', ['post' => $post->id]) }}" method="post"
                                        id="unlikeForm_{{ $post->id }}" class="mb-0">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="border-0 p-0" style="background:none;">
                                            <span class="like-btn">
                                ...
                                @else
                                    ...
                                    <form action="{{ route('posts.like', ['post' => $post->id]) }}" method="post"
                                        id="likeForm_{{ $post->id }}" class="mb-0">
                                        @csrf
                                        <button type="submit" class="border-0 p-0" style="background:none;">
                                ...
                                @endif
                            @endauth
                            <label for="comment_{{ $post->id }}" class="comment-btn"><svg aria-label="Comment"
                                    class="x1lliihq x1n2onr6 x5n08af" fill="currentColor" height="24" role="img" loading="lazy"
                                    viewBox="0 0 24 24" width="24">
                        ...
                        @auth
                            @if (auth()->user()->hasSaved($post->id))
                                ...
                                <form loading="lazy" action="{{ route('saved-posts.destroy', $post->id) }}" method="post"
                                    id="unsaveForm{{ $post->id }}" class="mb-0">
                                    @csrf
                                    @method('DELETE')
                                    <button loading="lazy" type="submit" class="border-0 p-0" style="background:none;">
                                        <span class="save-btn">
                                            <svg aria-label="Save" class="x1lliihq x1n2onr6 x5n08af" fill="currentColor"
                                                height="24" role="img" viewBox="0 0 24 24" width="24" loading="lazy">
                                                <title>Unsave</title>
                                                <polygon fill="#000" points="20 21 12 13.44 4 21 4 3 20 3 20 21"
                                                    stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                                                    stroke-width="2" loading="lazy"></polygon>
                                            </svg>
                                        </span>
                                    </button>
                            ...
                            @else
                                ...
                                <form loading="lazy" action="{{ route('saved.posts.store', ['id' => $post->id]) }}" method="post"
                ...
                <div class="likes mt-1"><span class="fw-bold">
                            ...
                            <ul class="list-unstyled">
                                <li class="d-flex flex-column ">
                                    <div class="d-flex align-items-center ">
                                        ...
                                        <div class="me-2 p-0">
                                            <img loading="lazy" src="{{ $comment->users->profiles->avatar ? Storage::url($comment->users->profiles->avatar) : url('https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_640.png') }}"
                                                    ...
                                                    <button loading="lazy" type="submit" class="border-0 p-0" style="background:none;">
                                                        <span class="like-btn">
                                                            <svg aria-label="Like" class="x1lliihq x1n2onr6 xyb1xck" fill="#ed4956" height="24" role="img" loading="lazy" viewBox="0 0 24 24" width="24">
                                                                ...
                                                                <!-- Inner heart fill -->
                                                                <path loading="lazy" fill="#ed4956" d="M12 21.35l-1.45-1.32C5.55 15.36 2 12.28 2 9.5 2 6.42 4.42 4 7.5 4c1.74 0 3.41.81 4.5 2.09C14.09 4.81 15.76 4 17.5 4 20.58 4 23 6.42 23 9.5c0 2.78-3.55 5.86-8.55 10.54L12 21.35z">
                                                                </path>
                                                            </svg>
                                                        </span>
                                                    </button>
                                    ...
                                    <div class="d-flex align-items-center justify-content-between">
                                        ...
                                        <form method="POST" action="{{ route('posts.comment.destroy', $comment->id) }}"
                                            ...
                                            @csrf
                                            @method('DELETE')
                                            <button loading="lazy" type="submit"
